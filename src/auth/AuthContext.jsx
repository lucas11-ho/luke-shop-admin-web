...
const activeScopeStores=user=>safeList(user?.store_scope?.stores).filter(store=>store?.status==='ACTIVE');
const permissionsOf=user=>safeList(user?.permissions);
export function AuthProvider({children}){
  ...
  const login=async({tenantSlug,email,password})=>{
    ...
    if(!u||!t?.access_token||!t?.refresh_token)throw new Error('Merchant login returned an invalid session response.');
