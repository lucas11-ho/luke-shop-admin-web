...
export function DriverAccessPage(){
 ...
 const activeStaff=useMemo(()=>staff.filter(x=>x.status==='ACTIVE'),[staff]);const linked=drivers.filter(x=>x.merchant_user_id).length;
 ...
 return <div className="vben-access-page" data-testid="driver-login-access-v1">
  ...
  <VbenAlert tone="info" title="Dedicated Driver App login">Driver URL: <strong>{location.origin}/#/driver</strong>. Username is the linked staff email. Password is that staff account's existing Merchant password. The driver opens directly into Home / Jobs / Messages / Cash / Me with no Merchant Admin sidebar.</VbenAlert>
  ...
  <VbenCard title="Driver login linkage" description="Linking changes only authentication identity. Operational permissions remain enforced by the Backend driver-scoped routes.">
   {loading?<VbenSkeleton lines={6}/>:<VbenTable rows={drivers} keyField="id" emptyTitle="No delivery drivers" emptyDescription="Create a driver in Delivery → Drivers first." columns={[
    ...
    {key:'current',label:'Driver App login',render:d=>d.merchant_user_email?<div><strong>{d.merchant_user_email}</strong><small style={{display:'block',color:'#64748b',marginTop:3}}>Use at /#/driver</small></div>:<VbenBadge tone="warning">NO LOGIN LINKED</VbenBadge>},
  ...
  <VbenCard title="Driver onboarding" description="Passwords remain hashed and cannot be viewed after creation."><div className="detail-grid"><div><span>1. Staff account</span><strong>Settings → Access → Staff</strong></div><div><span>2. Link driver</span><strong>Choose staff account above</strong></div><div><span>3. Driver URL</span><strong>{location.origin}/#/driver</strong></div><div><span>4. Install</span><strong>Add Driver App to phone Home Screen</strong></div></div>{canStaffManage&&<div className="right"><VbenButton onClick={()=>navigate('/access')}>Manage staff accounts</VbenButton></div>}</VbenCard>
