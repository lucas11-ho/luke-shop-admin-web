...
export function InventoryPage(){
 ...
 const submit=async(e)=>{e.preventDefault();setBusy(true);setErr(null);try{const quantity=validateMovement();await api.request('/v1/merchant/inventory/adjustments',{method:'POST',body:{...form,quantity,location_id:form.location_id||undefined}});setOpen(false);setForm(blankAdjustment);setToast('Inventory adjusted');await load();if(tab==='ledger')await loadLedger(form.inventory_item_id)}catch(e){setErr(e)}finally{setBusy(false)}};
