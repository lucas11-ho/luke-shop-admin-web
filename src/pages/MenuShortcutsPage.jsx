...
export function MenuShortcutsPage(){
 ...
 const canManage=has('customer_experience.manage');
 ...
 const columns=[
  ...
  {key:'title',label:'Shortcut',render:row=>canManage?<button type="button" className="vben-product-category-link" onClick={()=>setEdit({...row})}><strong>{row.title}</strong><small>{destinationLabel(row.destination)}</small></button>:<span className="vben-product-category-link is-readonly"><strong>{row.title}</strong><small>{destinationLabel(row.destination)}</small></span>},
