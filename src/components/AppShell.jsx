...
const navGroups=[
 {key:'commerce',label:'Commerce',icon:'orders',items:[['orders','Orders','orders.read','orders','nav.orders'],['products','Products','catalog.read','products','nav.products'],['categories','Categories','catalog.read','products',null],['inventory','Inventory','inventory.read','inventory','nav.inventory']]},
 ...
 {key:'storefront',label:'Storefront',icon:'experience',items:[['customer-experience','Customer Experience','customer_experience.read','experience','nav.experience'],['theme-system','Theme System','customer_experience.read','experience',null],['media-library','Media Library','catalog.read','media','nav.media'],['languages','Languages','customer_experience.read','languages','nav.languages'],['address-form','Address Form','customer_experience.read','address','nav.address']]},
 ...
 {key:'settings',label:'Settings',icon:'settings',items:[['settings','Store Settings','tenant.settings.read','settings','nav.settings'],['access','Staff & Access',null,'access','nav.access'],['staff-store-access','Staff store access','merchant.staff.read','access',null],['staff-notifications','Staff notifications','staff.notifications.read','support',null],['audit','Audit log','audit.read','audit','nav.audit'],['my-profile','My profile',null,'profile','nav.profile']]},
];

const iconPaths={
 dashboard:['M3 3h7v7H3z','M14 3h7v7h-7z','M3 14h7v7H3z','M14 14h7v7h-7z'],stores:['M4 10h16','M5 10V6h14v4','M6 10v9h12v-9','M9 19v-5h6v5'],orders:['M6 3h12l2 4-2 4H6L4 7z','M7 11v10h10V11','M9 15h6','M9 18h4'],products:['M4 7.5 12 3l8 4.5v9L12 21l-8-4.5z','M12 12 4.2 7.6','M12 12l7.8-4.4','M12 12v9'],media:['M4 5h16v14H4z','m7 13 3-3 5 5 2-2 3 3','M9 9h.01'],inventory:['M4 5h16v5H4z','M4 14h16v5H4z','M8 7.5h4','M8 16.5h6'],customers:['M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2','M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8','M22 21v-2a4 4 0 0 0-3-3.87','M16 3.13a4 4 0 0 1 0 7.75'],loyalty:['M12 3l2.6 5.3 5.9.9-4.3 4.2 1 5.9-5.2-2.8-5.2 2.8 1-5.9-4.3-4.2 5.9-.9z'],promotions:['M20 12 12 20 4 12 12 4z','M9 9h.01','M15 15h.01','M9 15l6-6'],payments:['M3 6h18v12H3z','M3 10h18','M7 15h3'],delivery:['M3 7h11v10H3z','M14 10h4l3 3v4h-7z','M6 17a2 2 0 1 0 4 0','M17 17a2 2 0 1 0 4 0'],experience:['M12 3l1.9 4.8L19 9l-4 3.3 1.2 5.2L12 15l-4.2 2.5 1.2-5.2L5 9l5.1-1.2z'],languages:['M4 5h10','M9 3v2c0 5-2 8-5 10','M6 9c1 2 3 4 6 5','M14 19l4-10 4 10','M15.5 16h5'],address:['M12 21s6-5.3 6-11a6 6 0 1 0-12 0c0 5.7 6 11 6 11z','M12 12a2 2 0 1 0 0-4 2 2 0 0 0 0 4'],support:['M4 5h16v11H7l-3 3z','M8 9h8','M8 12h5'],profile:['M12 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8','M4 21a8 8 0 0 1 16 0'],settings:['M12 8a4 4 0 1 0 0 8 4 4 0 0 0 0-8','M3 12h2','M19 12h2','M12 3v2','M12 19v2','m5.6-12.4 1.4 1.4','m17 17 1.4 1.4','m18.4 5.6-1.4 1.4','M7 17l-1.4 1.4'],access:['M12 3 4 7v5c0 4.8 3.4 7.7 8 9 4.6-1.3 8-4.2 8-9V7z','M9 12l2 2 4-4'],audit:['M5 4h14v16H5z','M8 8h8','M8 12h8','M8 16h5'],menu:['M4 7h16','M4 12h16','M4 17h16'],collapse:['M15 6l-6 6 6 6'],expand:['M9 6l6 6-6 6'],chevron:['M9 6l6 6-6 6'],user:['M12 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8','M4 21a8 8 0 0 1 16 0'],logout:['M10 17l5-5-5-5','M15 12H3','M21 19V5a2 2 0 0 0-2-2h-6'],home:['M3 11 12 4l9 7','M5 10v10h14V10','M9 20v-6h6v6'],
};
...
export function AppShell({route,children}){
 ...
 const[mobile,setMobile]=useState(false),[collapsed,setCollapsed]=useState(initialCollapsed),[stores,setStores]=useState([]),[accountOpen,setAccountOpen]=useState(false),[openGroup,setOpenGroup]=useState(()=>initialGroup(route));const notifications=useMerchantNotifications();
 useEffect(()=>{setMobile(false);setAccountOpen(false);const group=activeGroup(route);if(group)setOpenGroup(group)},[route]);
 useEffect(()=>{try{localStorage.setItem('luke-shop-admin.sidebar-collapsed',collapsed?'1':'0')}catch{}},[collapsed]);
 useEffect(()=>{try{if(openGroup)localStorage.setItem('luke-shop-admin.sidebar-group',openGroup)}catch{}},[openGroup]);
 ...
 return <div className={`vben-shell ${collapsed?'is-collapsed':''} ${mobile?'is-mobile-open':''}`}>
  ...
  <section className="vben-workspace">
   ...
   <div className="vben-tabbar"><button className={dashboardActive?'active':''} onClick={()=>go('dashboard')}><Icon name="home" size={14}/><span>{t('nav.dashboard')}</span></button>{!dashboardActive&&<button className="active"><Icon name={found?.[3]||'dashboard'} size={14}/><span>{title}</span></button>}</div>
